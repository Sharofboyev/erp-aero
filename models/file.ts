...
export function deleteFile(id: string): Promise<void>{
    return new Promise((resolve, reject) => {
        connection.query("DELETE FROM files WHERE id = ?", 
            [id], (err) => {
                if (err) return reject(err);
                return resolve();
            })
...
export function getFiles(limit: number, offset: number): Promise<void>{
    return new Promise((resolve, reject) => {
        connection.query("SELECT * FROM files ORDER BY pkey ASC LIMIT ? OFFSET ?", [limit, offset], (err, results) => {
                if (err) return reject(err);
...
export function getFile(id: string): Promise<any>{
    return new Promise((resolve, reject) => {
        connection.query("SELECT * FROM files WHERE id = ?", [id], (err, results) => {
                if (err) return reject(err);
                return resolve(results);
            })
    })
